...
part = function part_send_evm(is_logged, coin) {
  let coin_pre = part_send_evm.coin;
  part_send_evm.coin = coin;
  part_send_evm.now_waiting = false;
  part_send_evm.now_sending = false;

  $( '#form-web3-span-coin' ).text(coin);

  part_send_evm.update();
};
//ych.gui.parts["send"]["evm"] = part;

part.update = async function() {
    if (xybot.current_coin == "" || xybot.current_coin == undefined) {
    $( '#form-web3-span-coin' ).text("-");
    $( '#form-web3-evm-span-send-coin' ).text("-");
    return;
  }

  const coininfo = ych.data.coininfos[xybot.current_coin];
  ...
  if (coininfo.type != "evm_t1" && coininfo.type != "erc20_t1") {
    $( '#form-web3-div-addr-evm_t1' ).hide();
    $( '#form-web3-button-bcast' ).prop("disabled",true);
    return;
  }

  if (!(xybot.current_coin in ych.address) || ych.address[xybot.current_coin] == "") {
    $( '#form-web3-div-addr-evm_t1' ).hide();
    $( '#form-web3-button-bcast' ).prop("disabled",true);
    return;
  }

  $( '#form-web3-div-addr-evm_t1' ).show();

  $( '#form-web3-span-net-name' ).text(coininfo.cfg.net+" ChainId="+coininfo.cfg.netid);
  $( '#form-web3-but-switch-net' )
    .text("Switch network to "+coininfo.cfg.net+" ChainId="+coininfo.cfg.netid);

  // check ethereum
  if (typeof window.ethereum == 'undefined') {
    $( '#form-web3-span-provider-detected' ).text("NO");
    $( '#form-web3-span-provider-chainid' ).text("-");
    $( '#form-web3-div-provider' ).hide();
    $( '#form-web3-button-bcast' ).prop("disabled",true);
    return;
  }

  $( '#form-web3-div-provider' ).show();
  $( '#form-web3-span-provider-detected' ).text("YES");
  $( '#form-web3-span-provider-chainid' ).text(window.ethereum.chainId);
  if (parseInt(window.ethereum.chainId, 16) != coininfo.cfg.netid) {
    $( '#form-web3-but-switch-net' ).show();
    $( '#form-web3-div-account' ).hide();
    $( '#form-web3-button-bcast' ).prop("disabled",true);
    return;
  }

  // on right network
  $( '#form-web3-but-switch-net' ).hide();
  $( '#form-web3-div-account' ).show();

  let account = await part.get_ethereum_account_address();
  //console.log("get_ethereum_account", account);

  if (account == "") {
    $( '#form-web3-span-account' ).text("-");
    $( '#form-web3-but-connect-account' ).show();
    $( '#form-web3-button-bcast' ).prop("disabled",true);
    return;
  }

  // has account connected
  $( '#form-web3-but-connect-account' ).hide();
  let short = account.substr(0,6)+"..."+account.substr(account.length-4,account.length);
  $( '#form-web3-span-account' ).text(short);
  $( '#form-web3-button-bcast' ).prop("disabled",false);
};

part.check_ethereum_netid = function() {
  part.update(xybot.current_coin);
};
...
$( '#form-web3-but-switch-net' ).click( function( event ) {
  event.preventDefault();
  $( '#form-web3-but-switch-net' ).prop("disabled",true);
  const coininfo = ych.data.coininfos[xybot.current_coin];
...
part.switch_ethereum_net = async function(netid) {
  ...
  $( '#form-web3-but-switch-net' ).prop("disabled",false);
  setTimeout(function () { // lets 1 sec wait
    part.check_ethereum_netid();
  }, 1000);
};

$( '#form-web3-but-connect-account' ).click( function( event ) {
  event.preventDefault();
  $( '#form-web3-but-connect-account' ).prop("disabled",true);
...
part.request_ethereum_account = async function() {
  try {
    ...
    if (accounts.length ==0) {
      $( '#form-web3-span-account' ).text("<no_address>");
      $( '#form-web3-but-connect-account' ).prop("disabled",false);
      return;
    }
    let account = accounts[0];
    if (account == "") {
...
part.clear_error = function() {
  $( '#form-web3-box-err' ).css("background-color", "rgba(0,0,0,0)");
  $( '#form-web3-box-err' ).text('\u00A0');
};

part.show_error = function(err) {
  $( '#form-web3-box-err' ).css("background-color", "rgba(255,0,0,0.25)");
  $( '#form-web3-box-err' ).text(err);
};

part.dialog_web3_wait = $('#form-web3-wait');
...
part.form_web3_wait = part.dialog_web3_wait.find('form').on("submit", function(event) {
  event.preventDefault();
  part.dialog_web3_wait.modal("hide");
  part.now_waiting = false;
});
...
part.on_withdraw_wait = async function(withdraw) {
  ...
  $( '#form-web3-wait-div-receiver' ).text(withdraw.addr);
  $( '#form-web3-wait-span-quantity-coin' ).text(withdraw.coin);
  $( '#form-web3-wait-div-quantity' ).html(ych.gui.format_amount(withdraw.vals.amount));
  $( '#form-web3-wait-div-usd' ).html(ych.gui.format_usd((Number(withdraw.vals.amount) / 1.e8 *usdrate).toFixed(2)));

  part.now_waiting = true;
  part.now_sending_withdraw = withdraw;
  part.dialog_web3_wait.attr('title', "Wait. Preparing to broadcast: "+"withdraw-"+withdraw.gidx);
  part.dialog_web3_wait.modal("show");
}

part.dialog_web3 = $('#form-web3');
...
part.form_web3 = part.dialog_web3.find('form').on("submit", function(event) {
  event.preventDefault();
  part.dialog_web3.modal("hide");
  part.now_sending = false;
});

part.on_withdraw_bcast = async function(withdraw) {
  //console.log("on_withdraw_bcast", withdraw);
  ...
  part.dialog_web3.attr('title', "Ready to broadcast: "+"withdraw-"+withdraw.gidx);
  part.dialog_web3.modal("show");
...
part.on_withdraw_bcast_go = async function() {
  ...
  try {
    ...
    console.log("x2");
    const user = ych.address[xybot.current_coin];
...
//    }
    ...
    const tx = await part.vault.withdraw1(coininfo.cfg.coin, user, dest, uses, sig2, tw).then((res1) => {
      ...
      $.ajax({
        type: "PUT",
        url: "/u/withdraw_evm_txid",
        data: JSON.stringify(
          {
            coin: xybot.current_coin,
            gidx: withdraw.gidx,
            txid: res1.hash
          }, (key, value) =>
